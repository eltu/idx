...
func buildStatusPanelContent(daemonRepo ports.DaemonRepository, data statusPanelData) string {
...
func daemonStatusLine(daemonRepo ports.DaemonRepository, projectRoot string) string {
	if daemonRepo == nil {
		return statusMutedStyle.Render("— not configured")
	}

	state, err := daemonRepo.ReadState()
	...
	for _, project := range state.Projects {
		...
		since := project.StartedAt.Local().Format("15:04")
		return statusSuccessStyle.Render("✅ watching") + "  " + statusMutedStyle.Render(fmt.Sprintf("(PID %d, since %s)", project.PID, since))
	}
