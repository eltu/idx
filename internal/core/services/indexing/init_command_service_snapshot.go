...
func newChecksumSnapshot(fileEntries []domain.DirectoryEntry) ports.DirectoryChecksumSnapshot {
	return ports.DirectoryChecksumSnapshot{Files: make(map[string]ports.FileChecksumState, len(fileEntries))}
}

func snapshotLengthChanged(stored ports.DirectoryChecksumSnapshot, fileEntries []domain.DirectoryEntry) bool {
	return len(stored.Files) != len(fileEntries)
}

func (service InitCommandService) updateSnapshotEntry(currentFiles map[string]ports.FileChecksumState, entry domain.DirectoryEntry, storedFiles map[string]ports.FileChecksumState, changedFileNames map[string]struct{}) (bool, error) {
...
func reuseStoredChecksum(entry domain.DirectoryEntry, storedState ports.FileChecksumState, exists bool) bool {
	return exists && metadataUnchanged(entry, storedState) && storedState.Checksum != ""
}

func snapshotEntryChanged(exists bool, storedState ports.FileChecksumState, checksum string) bool {
	if !exists {
		return true
	}
