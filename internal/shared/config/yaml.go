...
// YAMLRepository reads .idx.yml from the project root.
// Example: cfg, overrides, err := NewYAMLRepository().Load(projectRoot).
...
func (r *YAMLRepository) FilePath(projectRoot string) string {
...
func (r *YAMLRepository) Load(projectRoot string) (IdxConfig, []string, error) {
	...
	if err != nil {
		return DefaultIdxConfig(), nil, fmt.Errorf("invalid %s at %q: %w", idxConfigFileName, path, err)
	}
...
func applyYAMLOverrides(cfg *IdxConfig, raw yamlIdxConfig) ([]string, error) {
...
func applySearchOverrides(cfg *SearchConfig, s *yamlSearchConfig) ([]string, error) {
	var keys []string
...
func applyWatchOverrides(cfg *WatchConfig, w *yamlWatchConfig) ([]string, error) {
	if w.Debounce == nil {
		return nil, nil
	}
...
func applyIndexOverrides(cfg *IndexConfig, i *yamlIndexConfig) []string {
	if i.Ignore == nil {
		return nil
	}
	cfg.Ignore = i.Ignore
...
func applyBM25Overrides(cfg *BM25Config, b *yamlBM25Config) []string {
	var keys []string
...
func applyLogOverrides(cfg *LogConfig, l *yamlLogConfig) []string {
	if l.Level == nil {
		return nil
	}
	cfg.Level = *l.Level
