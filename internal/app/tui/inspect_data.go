...
func inspectBuildLogRow(line string, position int, filePath string) inspectLogRow {
	...
	row = applyJSONToLogRow(row, line)
...
func applyJSONToLogRow(row inspectLogRow, line string) inspectLogRow {
	...
	if parsedPath := inspectStringField(parsed, "path", "file_path", "filePath", "directory"); parsedPath != "" {
		row.path = parsedPath
	}
	...
	if pretty, marshalErr := json.MarshalIndent(parsed, "", "  "); marshalErr == nil {
		row.jsonRaw = string(pretty)
	}
	return row
}

func inspectRowsFromIndex(index *indexing.InvertedIndex) ([]inspectDirectoryRow, map[string][]inspectDocumentRow) {
	if index == nil {
...
func indexDocumentsByDirectory(index *indexing.InvertedIndex) map[string][]inspectDocumentRow {
...
func documentTermCount(index *indexing.InvertedIndex, documentName string) int {
	count := 0
