...
func NewReadCommandService(projectTree filesystem.ProjectTree, streamer fileStreamer, output output.Writer) ReadCommandService {
...
func (service ReadCommandService) validateDependencies() error {
	if service.projectTree == nil {
		return fmt.Errorf("failed to run read command: got nil projectTree dependency, expected non-nil filesystem.ProjectTree")
	}
	...
	if service.output == nil {
		return fmt.Errorf("failed to run read command: got nil output dependency, expected non-nil output.Writer")
	}
