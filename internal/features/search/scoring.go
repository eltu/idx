...
func scoreDocuments(index *indexing.InvertedIndex, terms []string, operator string, tuning searchTuning) map[string]float64 {
...
func applyProximityBonus(scores map[string]float64, index *indexing.InvertedIndex, terms []string, matchingDocuments map[string]struct{}, tuning searchTuning) {
...
func applyTermCoverageMultiplier(scores map[string]float64, index *indexing.InvertedIndex, terms []string, matchingDocuments map[string]struct{}) {
	if len(terms) == 0 {
		return
	}
...
func termCoverage(index *indexing.InvertedIndex, filePath string, terms []string) float64 {
	matchedTerms := matchedTermCount(index, filePath, terms)
	return float64(matchedTerms) / float64(len(terms))
}

func matchedTermCount(index *indexing.InvertedIndex, filePath string, terms []string) int {
	count := 0
	for _, term := range terms {
...
func proximityBonusForDocument(index *indexing.InvertedIndex, filePath string, terms []string, tuning searchTuning) float64 {
	if len(terms) < 2 {
		return 0
	}
...
func documentsContainingAllTerms(index *indexing.InvertedIndex, terms []string) map[string]struct{} {
	if len(terms) == 0 {
...
func documentsContainingAnyTerm(index *indexing.InvertedIndex, terms []string) map[string]struct{} {
	matchingDocuments := make(map[string]struct{})
	for _, term := range terms {
		termStats := index.Terms[term]
		if termStats == nil {
...
func filterDocumentsByTerm(matchingDocuments map[string]struct{}, termStats *indexing.TermStats) {
	for filePath := range matchingDocuments {
...
func addTermScores(scores map[string]float64, index *indexing.InvertedIndex, termStats *indexing.TermStats, matchingDocuments map[string]struct{}, tuning searchTuning) {
	for filePath, docTerm := range termStats.Docs {
		...
		scores[filePath] += indexing.BM25Score(docTerm.TF, termStats.IDF, docStats.Length, index.AverageDocLength, tuning.bm25K1, tuning.bm25B)
	}
}
...
// indexing.TokenizeFileName, which handles '_', '.', '-', and CamelCase splits.
func fileNameTokens(fileName string) []string {
	domainTokens := indexing.TokenizeFileName(filepath.Base(fileName))
