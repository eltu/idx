...
func (service InitCommandService) syncEligibleDirectories(directories []string, projectRoot string, matcher filesystem.IgnoreMatcher) error {
...
func (service InitCommandService) loadMergedInspectIndex(directoryPaths []string) (*InvertedIndex, error) {
	sortedDirectories := append([]string(nil), directoryPaths...)
	sort.Strings(sortedDirectories)

	indicesByDirectory := make(map[string]*InvertedIndex, len(sortedDirectories))
...
func mergeInspectIndices(indicesByDirectory map[string]*InvertedIndex) *InvertedIndex {
...
func sortedIndexDirectories(indicesByDirectory map[string]*InvertedIndex) []string {
...
func mergeInspectIndexDirectory(target *InvertedIndex, directoryPath string, index *InvertedIndex) {
	if index == nil {
		return
	}
...
func mergeInspectDocuments(target *InvertedIndex, directoryPath string, index *InvertedIndex) {
	for documentName, stats := range index.Documents {
		if stats == nil {
			continue
		}
		documentID := inspectDocumentID(directoryPath, documentName)
		target.Documents[documentID] = &DocStats{Name: stats.Name, Path: stats.Path, Length: stats.Length}
	}
}

func mergeInspectTerms(target *InvertedIndex, directoryPath string, index *InvertedIndex) {
	for term, termStats := range index.Terms {
		...
		if targetTerm == nil {
			targetTerm = &TermStats{Docs: make(map[string]*DocTermStats)}
...
func cloneInspectDocTermStats(docTermStats *DocTermStats) *DocTermStats {
	if docTermStats == nil {
		return &DocTermStats{}
	}
