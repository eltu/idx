...
func TestInitCommandServiceBuildAndSaveIndexErrorBranches(t *testing.T) {
	...
	if err := service.buildAndSaveIndex(root, []filesystem.DirectoryEntry{entry}, snapshot, map[string]struct{}{}); err == nil {
...
func TestInitCommandServiceHelpersCoverUncoveredBranches(t *testing.T) {
	...
	if got := buildChangedLogEntries(nil, DirectoryChecksumSnapshot{Files: map[string]FileChecksumState{}}, map[string]struct{}{}, indexedAt); len(got) != 0 {
	...
	if sameSnapshotChecksums(map[string]FileChecksumState{"a": {Checksum: "1"}}, map[string]FileChecksumState{"a": {Checksum: "2"}}) {
		t.Fatal("expected sameSnapshotChecksums false when checksum differs")
	}

	matcher := &countingMatcher{}
	filtered, err := filterEntries([]filesystem.DirectoryEntry{{Name: "link", Path: filepath.Join(root, "link"), IsSymlink: true}, {Name: "file.txt", Path: filepath.Join(root, "file.txt")}}, root, matcher)
	if err != nil {
...
func TestInitCommandServiceDirectoryChecksumsAndSameChecksumsBranches(t *testing.T) {
	...
	checksums, err := service.directoryChecksums([]filesystem.DirectoryEntry{{Name: "a.txt", Path: firstPath}, {Name: "b.txt", Path: secondPath}})
	if err != nil {
	...
	service.fileReader = internalFileReader{err: errors.New("read failure")}
	if _, err := service.directoryChecksums([]filesystem.DirectoryEntry{{Name: "a.txt", Path: firstPath}}); err == nil {
...
func TestInitCommandServiceShouldReindexDirectoryAndMetadataBranches(t *testing.T) {
	...
	if metadataUnchanged(filesystem.DirectoryEntry{Size: 10, ModTimeUnixNano: 20}, FileChecksumState{Size: 10, ModTimeUnixNano: 20}) == false {
