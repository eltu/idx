# ADR 0008: Search Boolean Operator (AND / OR)
...
### 4. Nil-safety for proximity bonus in OR mode
...
- **Precision vs recall**: AND remains the default to preserve existing
  precision-first behaviour. OR is opt-in.
- **No index changes**: both operators work on the existing inverted index
  structure; no re-indexing is required.
- **Score semantics unchanged for AND**: the coverage multiplier and
  concentration tiebreaker are OR-only or additive; existing AND results are
  not affected.
- **Test coverage**: regression tests were added for the coverage multiplier,
  the concentration tiebreaker, and the CLI flag validation.
