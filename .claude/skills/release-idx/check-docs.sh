...
git diff "${LAST_TAG}..HEAD" --name-only -- '*.go' | grep -v '_test\.go' || echo "(none)"
echo ""

echo "--- CLI command files changed ---"
git diff "${LAST_TAG}..HEAD" --name-only -- 'internal/app/cli/*.go' | grep -v '_test\.go' || echo "(none)"
echo ""

echo "--- ADR files changed or added ---"
git diff "${LAST_TAG}..HEAD" --name-only -- 'docs/adr/*.md' || echo "(none)"
echo ""

echo "--- Feature docs changed ---"
git diff "${LAST_TAG}..HEAD" --name-only -- 'docs/features/*.md' || echo "(none)"
echo ""
